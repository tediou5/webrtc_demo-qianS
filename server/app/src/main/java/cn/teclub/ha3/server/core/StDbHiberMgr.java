...
public class  StDbHiberMgr extends StObject
{
	...
	public void close(){
		log.info("Close global session & session factory!");
		globalSession.close();
	...
	public List queryByCond(String query_str, QueryCB cb){
        ...
        log.debug("Query " + l.size() + " Records from DB -- Cost:" + util.getCostStr(MS_START));
        return l;
	}
	...
	public Object executeInNewSession( SessionCB cb){
        ...
        session.close();
        log.info("==== Cost (DB New Session): " + util.getCostMillis(MS_START) + "ms");
        return obj;
	}
	...
	public  Object executeInGlobalSession( SessionCB cb){
		...
		log.info("==== Cost (DB Global Session): " + util.getCostMillis(MS_START) + "/" + cost0 + "ms");
		return obj;
	}
	...
	public StModelClient queryClientByKey(final String cond, boolean load_friends){
		log.debug("HQL: " + cond + (load_friends ? ", With Friends" : ", Without Friends"));
        final long MS_START = System.currentTimeMillis();
        ...
        if(l.size() > 0){
        	...
        	if(load_friends){
        		...
        		log.info("#### Make sure friends are loaded!");
        		mc.getFriendList();
        	}
        }else{
        	mc = null;
        ...
        log.debug("Query Client -- Cost:" + util.getCostStr(MS_START));
		return mc;
	}
	...
	{
	  	log.debug("Query " + table +" by KEY(" + key_name + ")=0x" + Long.toHexString(key_value));
	...
	public void update(final StDbTable db_record){
		...
		final long MS_START = System.currentTimeMillis();
		//log.debug("Updating: " + db_record.dump());
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		session.update(db_record);
		session.getTransaction().commit();
		session.close();
		log.debug("Updated record: " + db_record + "-- Cost:" + util.getCostStr(MS_START));
		
		// [2016-9-4] db_record is NOT the one saved in DB. No need to log it!
		// log.debug("Updated record: " + db_record.toString());
	}
	...
	public void updateList(final StDbTable[]  records){
		...
		for(StDbTable obj : records){
			session.update(obj);
			log.debug("Updated record: " + obj);
		}
		session.getTransaction().commit();
		session.close();
		log.debug("Updated " + records.length + "records! -- Cost:" + util.getCostStr(MS_START));
	}
	...
	public void add(StDbTable  db_record){
		...
		final long MS_START = System.currentTimeMillis();
		//log.debug("Adding Record: " + db_record.dump());
        Session session = sessionFactory.openSession();
        session.beginTransaction();
        session.save(db_record);
        session.getTransaction().commit();
        session.close();
        log.debug("Added Record: " + db_record + " -- Cost:" + util.getCostStr(MS_START));
        //log.debug("Added Record: " + db_record.dump());
    }
	...
	public void delete(final StDbTable  db_record){
		...
		final long MS_START = System.currentTimeMillis();
		//log.debug("Deleting: " + db_record.dump());
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		session.delete(db_record);
		session.getTransaction().commit();
		session.close();
        log.debug("Deleted Record: " + db_record + " -- Cost:" + util.getCostStr(MS_START));
	}
